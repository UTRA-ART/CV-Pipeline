...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        def img2label_paths(img_paths):
            ...
            # print([x.replace(sa, sb, 1)[:len(x)-3] + 'txt' for x in img_paths])
            return [x.replace(sa, sb, 1)[: len(x) - 3] + "txt" for x in img_paths]
        ...
        bi = np.floor(np.arange(n) / batch_size).astype(np.uint)  # batch index
        ...
        if self.rect:
            ...
            self.batch_shapes = (
                np.ceil(np.array(shapes) * img_size / stride + pad).astype(np.uint)
                * stride
            )
            ...
            if l is not None and l.shape[0]:
                ...
                if extract_bounding_boxes:
                    ...
                    for j, x in enumerate(l):
                        ...
                        b[2:] = b[2:] * 1.3 + 30  # pad
                        b = xywh2xyxy(b.reshape(-1, 4)).ravel().astype(np.uint)
