...
transform = transforms.Compose(
    [transforms.ToPILImage(), transforms.Resize((180, 330)), transforms.ToTensor()]
)
...
for e in range(num_epochs):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        correct = torch.sum(
            torch.bitwise_and(masked_pred, labels_np.type(torch.int32))
        ).item()
        incorrect = torch.sum(
            torch.bitwise_or(masked_pred, labels_np.type(torch.int32))
        ).item()
        ...
        loss = criterion(out, labels) * 0.5 + 0.5 * dice_loss(
            (pred.detach() > 0.5).float(), labels
        )
        loss.backward()
        optimizer.step()
    ...
    print(
        "Training Error: {} | Training Loss: {} | Number of non-binary: {}".format(
            total_train_error / num_train_iterations,
            total_train_loss / num_train_iterations,
            num_not_binary,
        )
    )
