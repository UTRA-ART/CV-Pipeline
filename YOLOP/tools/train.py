...
def parse_args():
    ...
    parser.add_argument('--freeze_enc', type=int, default=5, help='# Layers to freeze in encoder')
    parser.add_argument('--freeze_LL', type=int, default=5, help='# Layers to freeze in LL')

    parser.add_argument('--TF', type=int, default=0, help='1 for TF')
...
def main():
    ...
    freeze_encoder_idx = [str(i) for i in range(0, min(0 + args.freeze_enc, 17) )]
    ...
    freeze_LL_idx = [str(i) for i in range(25,min(25+args.freeze_LL,34) )]
    ...
    if rank in [-1, 0]:
        ...
        if args.TF:  # Only train encoder and two segmentation branchs
            logger.info('freeze Det head...')
            for k, v in model.named_parameters():
                v.requires_grad = True  # train all layers 
                check_idx = k.split(".")[1]
                if check_idx in Det_Head_para_idx:
                    print('freezing %s' % k)
                    v.requires_grad = False
